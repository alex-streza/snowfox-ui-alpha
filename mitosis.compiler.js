...
function compile(filepath) {
  ...
  config.targets.forEach(async (target, i) => {
    ...
    const outFile = `${outPath}/${filepath.replace(`/${file.base}`, '')}.${
      config.extensions[i]
    }`;
    ...
    if (isFirstCompilation) {
      ...
      fs.copyFileSync('./tailwind.config.js', `${outPath}/tailwind.config.js`);
    ...
    if (target === 'angular') {
      ...
      const result = data
        .replace(
          ...
          }[sf-$1]", exportAs: "sf-$1", encapsulation: 2`
        )
        .replace(
          /\@Input\(\) className\: any\;/,
          '@Input() className: any;\n@Input() children: any;'
    ...
    if (target === 'webcomponent') {
      ...
      const result = data
        ...
        .replace(
          ...
          'customElements.get("sf-$1") || customElements.define("sf-$1", $2);'
        )
