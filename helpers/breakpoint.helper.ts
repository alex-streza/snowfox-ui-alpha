...
export function initBreakpointChecker() {
  ...
  const medias = breakpoints.map((breakpoint) => ({
    ...
    value: styles.getPropertyValue(`--sf-breakpoint-${breakpoint.value}`).trim()
  }));
  ...
  medias.forEach(({ key, value }) => {
    ...
    window
      ...
      .addEventListener('change', (e) => onChangeMedia(key, e.matches));
  });
}
